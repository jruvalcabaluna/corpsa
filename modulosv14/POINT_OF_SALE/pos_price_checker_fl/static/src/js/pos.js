                ...
                $('.screen-content-flexbox input').autocomplete({
                    ...
                    select: function (event, ui) {
                        if(ui != undefined){
                            ...
                            var default_pricelist = self.env.pos.default_pricelist.items;
                            var product_w_discount = default_pricelist.filter(p => p.product_tmpl_id[0] === product.product_tmpl_id).length > 0 ? default_pricelist.filter(p => p.product_tmpl_id[0] === product.product_tmpl_id)[0].price_discount : 0;
//                            var product_w_discount = default_pricelist.filter(p => p.product_tmpl_id[0] === product.product_tmpl_id).length > 0 ? default_pricelist.filter(p => p.product_tmpl_id[0] === product.product_tmpl_id)[0].percent_price : 0;
                            self.props.product_w_discount = product_w_discount;
                            self.render();
                        }
                    }
                });
