...
class POSConfigSummery(models.Model):
	...
	is_cash_out = fields.Boolean('Retirar Efectivo desde POS')
	is_cash_in = fields.Boolean('Ingresar Efectivo desde POS')
	is_print_statement = fields.Boolean('Print Statement Receipt')
	cash_threshold = fields.Float('Monto Umbral Permitido')
...
class POSSession(models.Model):
	...
	def get_cash_register_difference(self):
		...
		return cash_register_difference
	...
	def get_statement_data(self,stmt_st_date, stmt_end_date,selected_cashier):
		...
		for line in statements:
			...
			if line.amount > 0 :
				...
				data.update({'credit': line.amount, 'debit': '-', 'date':line.date, 'create_date': line.create_date})
				cash_in_data.append(data)
			else:
				debit_total += -(line.amount)
				data.update({'credit': '-', 'debit': -(line.amount), 'date':line.date, 'create_date': line.create_date})
...
class PosBoxIn(models.TransientModel):
	...
	def create_cash_in(self, user, reason, amount, session_id):
		...
		values = {
			...
			'name': stmt_id.name+'/'+cash_create.name,
