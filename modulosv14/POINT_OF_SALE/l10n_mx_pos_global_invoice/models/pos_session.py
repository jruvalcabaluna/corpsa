...
class PosSession(models.Model):
    ...
    def create_manual_global_invoice(self, records):
        ...
        data = {}
        print('El for del asiento contable esta correcto')
        print(data)
    ...
    def _reconcile_account_move_lines(self, data):
        ...
        print(split_cash_statement_lines)
        combine_cash_statement_lines = data.get('combine_cash_statement_lines')
        print(combine_cash_statement_lines)
...
class PosSessionGlobalWizard(models.TransientModel):
    _name = 'pos.session.global.wizard'
    _description = 'Creacion de factura global'

    def action_process_global_invoices(self):
        ctx = self._context.copy()
        active_ids = ctx.get('active_ids')
        model = ctx.get('active_model','')
        if model=='pos.session' and active_ids:
            sessions = self.env[model].browse(active_ids)
            #for session in sessions:
            #    try:
            self.env['pos.session'].sudo().create_manual_global_invoice(sessions)#edi_document_ids.filtered(lambda d: d.state in ('to_send', 'to_cancel'))._process_documents_web_services()
            #    except:
            #        pass
        return True
