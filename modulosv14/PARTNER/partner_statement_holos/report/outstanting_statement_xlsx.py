...
class OutstandingStatementXslx(models.AbstractModel):
    _name = "report.p_s.report_outstanding_statement_holos_xlsx"
    _description = "Outstanding Statement Holos XLSL Report"
    _inherit = "report.report_xlsx.abstract"

    def _get_report_name(self, report, data=False):
        company_id = data.get("company_id", False)
        report_name = _("Pendiente por Cobrar")
        if company_id:
    ...
    def _write_currency_lines(self, row_pos, sheet, partner, currency, data):
        ...
        row_pos += 2
        statement_header = _(
            "%sInforme hasta %s en %s"
            % (
                account_type == "payable" and _("Proveedor ") or "",
        ...
        row_pos += 1
        sheet.write(
            row_pos, 0, _("Folio"), FORMATS["format_theader_yellow_center"]
        )
        sheet.write(row_pos, 1, _("Fecha"), FORMATS["format_theader_yellow_center"])
        sheet.write(row_pos, 2, _("Fecha Vencimiento"), FORMATS["format_theader_yellow_center"])
        sheet.write(
            row_pos, 3, _("Folio"), FORMATS["format_theader_yellow_center"]
        )
        sheet.write(row_pos, 4, _("Original"), FORMATS["format_theader_yellow_center"])
        sheet.write(
            row_pos, 5, _("Importe Pendiente"), FORMATS["format_theader_yellow_center"]
        )
        sheet.write(row_pos, 6, _("Saldo"), FORMATS["format_theader_yellow_center"])
        ...
        sheet.merge_range(
            row_pos, 2, row_pos, 4, _("Saldo Final"), FORMATS["format_tcell_left"]
        )
        sheet.write(
    ...
    def _write_currency_buckets(self, row_pos, sheet, partner, currency, data):
        report_model = self.env["report.partner_statement_holos.outstanding_statement_holos"]
        partner_data = data.get("data", {}).get(partner.id, {})
        currency_data = partner_data.get("currencies", {}).get(currency.id)
        if currency_data.get("buckets"):
            row_pos += 2
            buckets_header = _("Informe de antigüedad al %s en %s") % (
                partner_data.get("end"),
                currency.display_name,
            )
            sheet.merge_range(
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        sheet.merge_range(
            ...
            row_pos,
            6,
            _("Estado de Cuenta %s" % (company.display_name)),
            FORMATS["format_ws_title"],
        )
        row_pos += 1
        sheet.write(row_pos, 1, _("Fecha:"), FORMATS["format_theader_yellow_right"])
        sheet.write(
            row_pos,
            2,
        ...
        for partner in partners:
            ...
            row_pos += 3
            sheet.write(
                row_pos, 1, _("Estado de Cuenta de:"), FORMATS["format_theader_yellow_right"]
            )
            sheet.merge_range(
                row_pos,
                2,
            ...
            row_pos += 1
            sheet.write(
                row_pos, 1, _("Compañía:"), FORMATS["format_theader_yellow_right"]
            )
            sheet.merge_range(
                row_pos,
                2,
