...
class ActivityStatementXslx(models.AbstractModel):
    _name = "report.p_s.report_activity_statement_holos_xlsx"
    _description = "Estado de Cuenta Cliente XLSL Holos Report"
    _inherit = "report.report_xlsx.abstract"

    def _get_report_name(self, report, data=False):
        company_id = data.get("company_id", False)
        report_name = _("Estado de Cuenta del Cliente")
        if company_id:
    ...
    def _write_currency_lines(self, row_pos, sheet, partner, currency, data):
        ...
        sheet.merge_range(
            row_pos, 2, row_pos, 4, _("Saldo Inicial"), FORMATS["format_tcell_left"]
        )
        sheet.write(
            row_pos,
            6,
    ...
    def _write_currency_buckets(self, row_pos, sheet, partner, currency, data):
        report_model = self.env["report.partner_statement_holos.activity_statement_holos"]
        partner_data = data.get("data", {}).get(partner.id, {})
        currency_data = partner_data.get("currencies", {}).get(currency.id)
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        sheet = workbook.add_worksheet(_("Estado de Cuenta"))
        sheet.set_landscape()
        row_pos = 0
        sheet.merge_range(
            row_pos,
        ...
        for partner in partners:
            ...
            row_pos += 3
            sheet.write(
                row_pos, 1, _("Estado de Cuenta:"), FORMATS["format_theader_yellow_right"]
            )
            sheet.merge_range(
                row_pos,
                2,
