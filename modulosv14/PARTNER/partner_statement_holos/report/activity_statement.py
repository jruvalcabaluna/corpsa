...
class ActivityStatement(models.AbstractModel):
    ...
    _inherit = "statement.common.holos"
    _name = "report.partner_statement_holos.activity_statement_holos"
    _description = "Estado de Cuenta Cliente"
    ...
    def _display_lines_sql_q1(self, partners, date_start, date_end, account_type):
        return str(
            self._cr.mogrify(
                """
            SELECT m.name AS move_id, sum(m.amount_residual) as amount_residual, l.matching_number as matching_number, l.partner_id, l.date,
                ...
                CASE
                    ...
                    ELSE ''
                END as ref,
                l.blocked, aj.name as journal, l.currency_id, l.company_id,
                sum(CASE WHEN (l.currency_id is not null AND l.amount_currency > 0.0)
                    THEN l.amount_currency
                    ELSE l.debit
                END) as debit,
            ...
            GROUP BY l.partner_id, m.name, l.date, l.date_maturity,
                ...
                CASE
                    ...
                    ELSE ''
                END,
                l.blocked, aj.name, l.matching_number, l.currency_id, l.company_id
        """,
                locals(),
            ),
            "utf-8",
    ...
    def _display_lines_sql_q2(self, company_id):
        return str(
            self._cr.mogrify(
                """
            SELECT Q1.partner_id, Q1.move_id, Q1.amount_residual, Q1.journal, Q1.matching_number, Q1.date, Q1.date_maturity,
    ...
    def _get_account_display_lines(
        ...
        WITH Q1 AS (%s),
             Q2 AS (%s)
        SELECT partner_id, move_id, amount_residual, journal, date, date_maturity, name, ref, debit,
                            credit, amount, blocked, currency_id
        FROM Q2
        ORDER BY matching_number, date, move_id"""

        # date, date_maturity, move_id
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if "company_id" not in data:
            wiz = self.env["activity.statement.holos.wizard"].with_context(
