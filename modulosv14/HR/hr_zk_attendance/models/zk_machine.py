...
class ZkMachine(models.Model):
    ...
    def download_attendance(self):
        ...
        for info in self:
            ...
            timeout = 15
            try:
                zk = ZK(machine_ip, port=zk_port, timeout=timeout, password=0, force_udp=True, ommit_ping=False)
            except NameError:
            ...
            if conn:
                ...
                if attendance:
                    for each in attendance:
                        print('EACH user_id', each.user_id)
                        print('EACH punch', each.punch)
                        print('EACH status', each.status)
