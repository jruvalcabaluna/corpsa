...
class SaleDetail(models.AbstractModel):
    _name = 'sale.detail'
    _description = 'Sale detail'
    _inherit = 'report.report_xlsx.abstract'

    cv = fields.Char('C.V.')
    agente = fields.Char('Agente')
    ...
    mes_vta = fields.Char('Mes Vta.')
    devolucion = fields.Char('Devolución')
    ajuste = fields.Char('Ajuste')
    garantia = fields.Char('Garantia')
    neta = fields.Char('Neto')
    total_vta = fields.Char('VTA.$')
    total_dev = fields.Char('DEV.$')
    total_aju = fields.Char('AJU.$')
    total_gar = fields.Char('GAR.$')
    total_desc = fields.Char('DESC.$')
    total_neto = fields.Char('NETO.$')
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.merge_range(5, 0, 5, 1, 'Almacen(es) : ', format4)
        w_house = w_house.join(get_warehouse[0])
        sheet.merge_range(5, 2, 5, 3+len(get_warehouse[0]), w_house, format4)
        user = self.env['res.users'].browse(self.env.uid)
        tz = pytz.timezone(user.tz)
        time = pytz.utc.localize(datetime.now()).astimezone(tz)
        sheet.merge_range('A8:G8', 'Fecha: ' + str(time.strftime("%Y-%m-%d %H:%M %p")), format1)
        sheet.merge_range(7, 7, 7, count, 'Almacenes', format1)
        sheet.merge_range('A9:G9', 'Informacion del Producto', format11)
        w_col_no = 6
        w_col_no1 = 7
        ...
        p_col_no1 = 7
        for i in get_warehouse[0]:
            sheet.write(9, p_col_no1, '', format21)#Available
            sheet.write(9, p_col_no1 + 1, 'Previsto', format21)
            sheet.write(9, p_col_no1 + 2, 'Entrante', format21)
            sheet.write(9, p_col_no1 + 3, 'Saliente', format21)
            sheet.merge_range(9, p_col_no1 + 4, 9, p_col_no1 + 5, 'Disponible', format21)
            ...
            sheet.write(9, p_col_no1 + 10, 'Valor', format21)
        ...
        for i in get_warehouse[1]:
            ...
            for each in get_line:
                if each['available'] < 0:
                    sheet.write(prod_row, prod_col, '', red_mark)#each['available']
                else:
                    sheet.write(prod_row, prod_col, '', font_size_8)#each['available']
                if each['virtual'] < 0:
