...
class ProductSet(models.Model):
    ...
    ref = fields.Char(string='Internal Reference',help='Product set internal reference',copy=False,)
    set_line_ids = fields.One2many('product.set.line', 'product_set_id', string="Products")
    company_id = fields.Many2one('res.company','Company',default=lambda self: self.env.user.company_id,ondelete='cascade',)
    partner_id = fields.Many2one(comodel_name="res.partner",required=False,ondelete="cascade",index=True,
