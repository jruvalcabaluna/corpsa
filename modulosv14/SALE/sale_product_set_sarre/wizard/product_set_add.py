...
class ProductSetAdd(models.TransientModel):
    ...
    order_id = fields.Many2one('sale.order', 'Sale Order', required=True,default=lambda self: self.env.context.get('active_id'),ondelete='cascade')
    partner_id = fields.Many2one(related='order_id.partner_id',ondelete='cascade')
    product_set_id = fields.Many2one('product.set', 'Product set',required=True,ondelete='cascade')
    quantity = fields.Float(digits=dp.get_precision('Product Unit of Measure'), required=True,default=1)
    days_ids = fields.Many2many('week.days', string="days of")

    skip_existing_products = fields.Boolean(default=False,
    ...
    def prepare_sale_order_line_data(self, set_line,
                                     max_sequence=0):
        self.ensure_one()
        m3_mensuales = (len(self.days_ids)* self.quantity)*4.3
        print(m3_mensuales)
        price_unit_s = 0
        if m3_mensuales < 90:
            price_unit_s = 180
        if m3_mensuales > 91 < 300:
            price_unit_s = 150
        if m3_mensuales > 300:
            price_unit_s = 110

        print(price_unit_s)
        price = m3_mensuales * price_unit_s
        print(price)
        sale_line = self.env['sale.order.line'].new({
            'order_id': self.order_id.id,
            'name': set_line.product_id.name +' '+str(m3_mensuales) + 'm3',
            'price_unit': price,
...
class WeekDays(models.Model):
    ...
    weak_days = fields.Many2many('product.set.add', 'days','b','a', string="Days")
