...
class PosCacheDatabase(models.Model):
    ...
    def request_pos_sessions_online_reload(self):
        sessions = self.env['pos.session'].sudo().search([
            ('state', '=', 'opened')
        ])
        for session in sessions:
            self.env['bus.bus'].sendmany(
                [[(self.env.cr.dbname, 'backend.request.pos.sync.datas', session.user_id.id), {}]])
        return True
    ...
    def get_modifiers_backend(self, write_date, res_model, config_id=None):
        ...
        if res_model != 'product.product':
            records = self.sudo().search([('write_date', '>', to_date), ('res_model', '=', res_model)], limit=1001)
            if len(records) > 1000:
                _logger.warning('[get_modifiers_backend] Your Backend Have change Partners row bigger 1000 rows. Please request admin go to POS Setting / Reinstall Database Cache')
            _logger.info('[get_modifiers_backend] %s partners need sync' % len(records))
            ...
            products = self.env[res_model].search([('write_date', '>', to_date)], limit=1001)
            if len(products) > 1000:
                _logger.warning('[get_modifiers_backend] Your Backend Have change Products row bigger 1000 rows. Please request admin go to POS Setting / Reinstall Database Cache')
            _logger.info('[get_modifiers_backend] %s products need sync' % len(products))
    ...
    def sync_products_partners(self, model_values, config_id=None):
        _logger.info('[BEGIN] sync_products_partners')
        results = {}
        ...
        _logger.info('[END] sync_products_partners')
        return results
    ...
    def remove_record(self, model, record_id):
        ...
        if records:
            ...
            self.create(vals)
        self.request_pos_sessions_online_reload()
        return True
