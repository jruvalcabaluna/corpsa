...
class PosSession(models.Model):
    ...
    def action_pos_session_closing_control(self):
        for session in self:
            if not session.config_id.allow_closing_session and not session.user_id.has_group(
    ...
    def create(self, vals):
        ...
        _logger.info('[Created] POS Session %s for user %s' % (session.name, self.env.user.login))
        return session

    def write(self, vals):
        res = super(PosSession, self).write(vals)
        for session in self:
            _logger.info('[Updated] POS Session %s from user %s' % (session.name, self.env.user.login))
        return res
