    ...
    models.PosModel = models.PosModel.extend({
        ...
        automation_backup_orders: function () {
            ...
            return rpc.query({
                ...
                setTimeout(_.bind(self.automation_backup_orders, self), 120000);
            });
        },
        ...
        async automation_push_order_one_by_one() {
            const orders = this.db.get_orders()
            try {
                if (orders.length == 1) {
                    this.set_synch('connecting', 'Saving Order ...');
                    let order_ids = await this._save_to_server(orders, {})
                    console.log('--> automatic pushed successfully order ids: ' + order_ids)
                } else {
            ...
            setTimeout(_.bind(this.automation_push_order_one_by_one, this), 5000);
        },
        load_server_data: async function () {
            const self = this;
            // TODO: dont remove this line, because pos session cached, all new orders will put back last session
            ...
            // -----------------------------------------------------------------
            if (!this.session.big_datas_turbo) {
                ...
                // TODO: loading pos session direct --------------
                const pos_session_id = this.session.config.pos_session_id;
                const domain = [
                    ['state', 'in', ['opening_control', 'opened']],
                    ['rescue', '=', false],
                    ['id', '=', pos_session_id]
                ];
                const sessionsData = await this.rpc({
                    model: 'pos.session',
                    method: 'search_read',
                    domain: domain,
                    fields: []
                ...
                // -----------------------------------
                this.next_load_index_by_model = {}
                this.datasCached = {}
                let progress = 0;
                let progress_step = 1.0 / self.models.length;
                let tmp = {}; // this is used to share a temporary state between models loaders
                let loaded = new Promise(function (resolve, reject) {
                    function load_model(index) {
                        if (index >= self.models.length) {
                            ...
                            let cond = typeof model.condition === 'function' ? model.condition(self, tmp) : true;
                            if (!cond) {
                                load_model(index + 1);
                                return;
                            }
                            ...
                            if (model.model) {
                                ...
                                if (self.json_datas && self.json_datas[model.model] && !self.models_load_realtime.includes(model.model)) {
                                    let results = self.json_datas[model.model];
                                    if (model.model == 'pos.session') {
                                        results = [sessionsData]
                                        console.warn('[load_server_data] loading POS Session Data')
                                    }
                                    let resultForLoaded = []
                                    if (results.length == 1) {
                                        resultForLoaded = results[0]
                                    }
                                    if (results.length > 1) {
                                        console.warn('Model ' + model.model + ' have total ' + results.length + ' results')
                                        if (!self.next_load_index_by_model[model.model]) {
                                            self.next_load_index_by_model[model.model] = 0
                                            resultForLoaded = results[0]
                                        } else {
                                            self.next_load_index_by_model[model.model] += 1
                                            resultForLoaded = results[self.next_load_index_by_model[model.model]]
                                        }
                                    }
                                    try { // catching exceptions in model.loaded(...)
                                        Promise.resolve(model.loaded(self, resultForLoaded, tmp))
                                            .then(function () {
                                                    load_model(index + 1);
                                                },
                                                function (err) {
                                    ...
                                    rpc.query(params).then(function (result) {
                                        let model = self.models[index];
                                        if (!self.datasCached[model.model]) {
                                            self.datasCached[model.model] = [result]
                                        } else {
                                            self.datasCached[model.model].push(result)
                                        }
                                        try { // catching exceptions in model.loaded(...)
                                ...
                                console.log('Label loading: ' + model.label)
                                try { // catching exceptions in model.loaded(...)
                ...
                return loaded.then(function () {
                    ...
                    self.pos_sync_pricelists = new exports.pos_sync_pricelists(self);
                    self.pos_sync_pricelists.start();
                    if (self.datasCached['product.addons'] && self.config.big_datas_turbo) {
                        let cached = [];
                        for (let model in self.datasCached) {
                            cached.push({
                                id: model,
                                value: self.datasCached[model]
                            });
                        }
                        if (cached.length) {
                            self.indexed_db.write('cached', cached, true);
                            console.log('SUCCESSFULLY STARTED POS SESSION AND UPDATED CACHED OF POS')
                        }
                    }
                    if (self.config.backup_orders_automatic) {
                        return self.restore_orders().then(function () {
        ...
        initialize: function (session, attributes) {
            ...
            this.automation_push_order_one_by_one() // starting schedule push order one by one
