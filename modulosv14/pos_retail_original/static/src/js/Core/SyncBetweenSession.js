odoo.define('pos_retail.synchronization', function (require) {
    ...
    exports.pos_bus = Backbone.Model.extend({
        ...
        set_online: function () {
            ...
            self.pos.set('syncStatus', {status: 'connecting', pending: '[Sync] Online '});
        },
        set_offline: function (total_error) {
            ...
            console.log('{set offline} with total_error: ' + total_error)
        ...
        syncing() {
            ...
            async function waitSync() {
                ...
                if (!self.pos.config.sync_multi_session) {
                    pending += 1
                    self.pos.set('syncStatus', {status: 'disconnected', pending: '[Syncing Pending] ' + pending + ' seconds'});
        ...
        send_notification: function (value, send_manual = false) {
            ...
            console.log('[send_notification] sending ' + action);
            this.pos.set('syncStatus', {
                status: 'connecting',
                pending: 'Sync [Start]'
            })
        ...
        repush_to_another_sessions: function () {
            let self = this;
            let datas_false = this.pos.db.get_datas_false();
    ...
    models.PosModel = models.PosModel.extend({
        ...
        get_notifications: function (message) {
            ...
            console.log('[get_notifications] get new action ' + action);
        ...
        sync_trigger_update_line: function (vals) {
            let self = this;
            let line = self.get_line_by_uid(vals['uid']);
            let order = self.get_order_by_uid(vals['line']['order_uid']);
            let json = vals['line'];
            if (line) {
                line.syncing = true;
                if (json.note) {
