...
odoo.define('pos_retail.order', function (require) {
    ...
    models.Order = models.Order.extend({
        ...
        set_tip: async function (tip) {
            ...
            if (!tip_product) {
                ...
                if (result) {
                    await this.pos.sync_products_partners();
                } else {
        ...
        init_from_JSON: function (json) {
            ...
            if (json.location_id) {
                ...
                if (location) {
                    ...
                    let location = this.get_picking_source_location();
                    this.set_picking_source_location(location)
                }
            } else {
                let location = this.get_picking_source_location();
        ...
        async suggestItems() {
            ...
            let {confirmed, payload: results} = await this.pos.chrome.showPopup('PopUpSelectionBox', {
            ...
            if (confirmed) {
                ...
                for (let index in selectedCrossItems) {
                    ...
                    if (product) {
                        ...
                        this.pos.chrome.showPopup('ErrorPopup', {
                            title: _t('Product not available in POS'),
                            body: _t('Contact your admin system, Product: ') + item['product_id'][1] + _t(' not available in POS')
                        })
                    }
                }
            }
        ...
        async setProductPackaging() {
            ...
            let {confirmed, payload: packSelected} = await this.pos.chrome.showPopup('SelectionPopup', {
        ...
        async submitOrderToBackEnd() {
            ...
            let {confirmed, payload: selectedItems} = await this.pos.chrome.showPopup(
                'PopUpSelectionBox',
                {
        ...
        set_client: async function (client) {
            ...
            if (client && !this.pos.the_first_load) {
                ...
                if (partial_payment_orders.length != 0) {
                    ...
                    if (this.pos.chrome) {
                        this.pos.chrome.showPopup('ConfirmPopup', {
                            ...
                            body: this.pos.env._t('Have total ') + partial_payment_orders.length + this.pos.env._t(' Orders not paid full and Due Amount Total is: ') + this.pos.format_currency(due_amount),
                            disableCancelButton: true,
                        })
                    }
                ...
                if (client.group_ids.length > 0) {
                    ...
                    if (lists.length > 0) {
                        const {confirmed, payload: group} = await this.pos.chrome.showPopup('SelectionPopup', {
                ...
                if (this.pos.coupons_by_partner_id && this.pos.coupons_by_partner_id[client.id] && this.get_total_with_tax() > 0) {
                    ...
                    const {confirmed, payload: coupon} = await this.pos.chrome.showPopup('SelectionPopup', {
        ...
        add_shipping_cost: function (service, product, is_shipping_cost) {
            if (service['type'] == 'fixed') {
                ...
                let amount_total = this.get_total_with_tax();
                let amount_tax = this.get_total_tax();
                let sub_amount = amount_total - amount_tax;
                let price = sub_amount - (sub_amount * service.amount / 100)
                this.add_product(product, {
                    price: price,
                    quantity: 1,
                    merge: false,
        ...
        async set_discount_value(discount) {
            ...
            if (discount > total_withtax) {
                ...
                if (this.pos.config.discount_limit && this.pos.config.discount_value_limit < discount) {
                    let confirm = await this.pos._validate_action(_t('Need approve add discount value'));
                    if (!confirm) {
                        return this.pos.chrome.showPopup('ErrorPopup', {
                        ...
                        let discount = discount / lines.length;
                        for (let i = 0; i < lines.length; i++) {
                            let line = lines[i];
        ...
        remove_orderline: async function (line) {
            if (this.pos.config.validate_remove_line && !this.syncing && !this.pos.the_first_load) {
                let validate = await this.pos._validate_action(this.pos.env._t('User requested approve delete line, if you accept POS Pas Pin, line selected will remove'));
                if (!validate) {
                    return false;
                }
            }
        ...
        add_product: async function (product, options) {
            ...
            let description = newDescription.join('/');
        ...
        update_product_price: function (pricelist) {
            let self = this;
            let products = this.pos.db.get_all_products();
            if (!products) {
                return;
            }
            for (let i = 0; i < products.length; i++) {
                let product = products[i];
                let price = this.pos.get_price(product, pricelist, 1);
                product['price'] = price;
            }
            ...
            if (reward['type'] == 'discount_products') {
                ...
                if (confirmed) {
                    ...
                    if (client['pos_loyalty_point'] < next_redeem_point) {
                        return this.pos.chrome.showPopup('ErrorPopup', {
                            title: _t('Warning'),
                            body: _t("Could not set points bigger than customer's points")
                        })
                    } else {
    ...
    models.Orderline = models.Orderline.extend({
        ...
        set_addons(addon_ids) {
            ...
            for (let index in addon_ids) {
                ...
                if (product) {
                    ...
                    let price = product.get_price(product, this.pos._get_active_pricelist(), this.quantity);
                    price_extra += price
                }
            }
            ...
            if (this.product.uom_ids && this.product.uom_ids.length) {
                ...
                for (let i = 0; i < this.product.uom_ids.length; i++) {
                    ...
                    if (uom && uom_has_price_included_pricelist) {
                        let price = this.pos.get_price(this.product, this.order.pricelist, 1, uom_id);
                        units.push({
                            uom: uom,
                            price: price
                        })
        ...
        _validate_stock_on_hand: function (quantity) {
            let line_quantity = quantity;
            let product = this.product;
            let stock_datas = this.pos.db.stock_datas;
            if (product['type'] == 'product' && stock_datas && stock_datas[product.id] != undefined) {
                if (!quantity) {
                    line_quantity = this.quantity;
                }
                let stock_available = stock_datas[product.id];
        ...
        set_quantity: function (quantity, keep_price) {
            ...
            if (this.pos.the_first_load == false && quantity != 'remove' && !this.pos.config['allow_order_out_of_stock'] && quantity && quantity != 'remove' && this.order.syncing != true && this.product['type'] == 'product') {
                const quantitiesByProduct = this.order.product_quantity_by_product_id()
                let nextQtyInCart = parseFloat(quantity)
                if (quantitiesByProduct[this.product.id]) {
                    nextQtyInCart += quantitiesByProduct[this.product.id]
                }
                const currentStockOnHand = this.pos.db.stock_datas[this.product.id];
                if (nextQtyInCart >= currentStockOnHand) {
                    let product = this.pos.db.get_product_by_id(this.product.id);
                    this.pos.alert_message({
                        title: _t('Warning'),
                        body: product['display_name'] + _t(' will out of Stock, Current stock is: ') + currentStockOnHand,
                    });
                }
            }
        ...
        async set_discount(discount) {
            ...
            if (!this.pos.the_first_load && this.pos.config.discount_limit && discount > this.pos.config.discount_limit_amount) {
                let validate = await this.pos._validate_action(_t(' Need approved set Disc: ') + discount + ' % .');
                if (!validate) {
                    return this.pos.chrome.showPopup('ErrorPopup', {
                        title: _t('Error'),
