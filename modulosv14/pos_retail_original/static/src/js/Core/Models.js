...
odoo.define('pos_retail.model', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        async automaticSetCoupon() {
            console.log('[automaticSetCoupon]')
            ...
            if (this.couponProgramsAutomatic && !lineAppliedCoupon) {
                ...
                for (let i = 0; i < this.couponProgramsAutomatic.length; i++) {
                    ...
                    if (canBeApplyCoupon) {
                        console.log('[canBeApplyCoupon] set Coupon Program: ' + canBeApplyCoupon['name'])
                        ...
                        if (hasAppliedCoupon) {
                            this.chrome.showPopup('ConfirmPopup', {
                                ...
                                body: this.env._t('Applied Promotion/Coupon: ') + couponProgram.name,
        ...
        async _applyCouponReward(program, coupon, rule, reward) {
            ...
            if (!product) {
                return this.chrome.showPopup('ErrorPopup', {
                    ...
                    body: reward.discount_line_product_id[1] + this.env._t(' not found. Can not apply reward'),
                    disableCancelButton: true,
                })
            }
            await this.rpc({
            ...
            if (coupon) {
                ...
                if (!['new', 'sent'].includes(coupon.state) || (coupon['is_gift_card'] && coupon['balance_amount'] <= 0)) {
                    return this.alert_message({
                        ...
                        body: coupon.code + this.env._t(', This coupon has already been used or expired')
                    })
                }
        ...
        async getInformationCouponPromotionOfCode(code) {
            ...
            if (!program && !coupon) {
                console.warn('Coupon is not found. This coupon does not exist or expired or used before')
                return false
            }
            if (program && rule && reward) {
                ...
                if (canBeApplyCoupon) {
                    ...
                    if (hasAppliedCoupon) {
                        this.chrome.showPopup('ConfirmPopup', {
                            title: this.env._t('Successfully'),
                            body: this.env._t('Applied Promotion/Coupon: ') + program.name,
                            disableCancelButton: true,
                        })
                        return true
                    }
        ...
        set_cashier: function (employee) {
            ...
            if (employee.is_employee && this.chrome && this.chrome.env && this.chrome.env.qweb) {
                ...
                this.config.allow_offline_mode = employee['allow_offline_mode']
            ...
            return _super_PosModel._save_to_server.call(this, orders, options).then(function (pos_order_backend_ids) {
                if (pos_order_backend_ids.length == 1) {
                    if (pos_order_backend_ids) {
                        ...
                        for (let i = 0; i < pos_order_backend_ids.length; i++) {
                            let backend_order = pos_order_backend_ids[i];
        ...
        load_server_data_by_model: function (model) {
            ...
            console.log('[load_server_data_by_model]: ' + model.model);
