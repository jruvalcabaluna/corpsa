...
odoo.define('pos_retail.database', function (require) {
    ...
    _super_db.add_products = function (products) {
        ...
        if (self.posmodel.config.limit_categories && self.posmodel.config.iface_available_categ_ids.length > 0) {
            products = products.filter(p => !p.pos_categ_id || (p.pos_categ_id && self.posmodel.config.iface_available_categ_ids.includes(p.pos_categ_id[0])))
        }
        // end custom
        // core odoo
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            if (product.available_in_pos) {
                ...
                if (product.addon_id && self.posmodel.addon_by_id && self.posmodel.addon_by_id[product.addon_id[0]]) {
                    product.addon = self.posmodel.addon_by_id[product.addon_id[0]]
                } else {
                    product.addon = null
                }
            }
    ...
    _super_db.add_partners = function (partners) {
        ...
        for (let i = 0, len = partners.length; i < len; i++) {
            ...
            if (partner.property_product_pricelist && self.posmodel.pricelist_by_id) { // re-update pricelist when pricelist change the same
                let pricelist = self.posmodel.pricelist_by_id[partner.property_product_pricelist[0]];
                if (pricelist) {
    ...
    db.include({
        ...
        get_all_products() {
            ...
            console.log('[get_all_products] total: ' + products.length)
            return products
        },
        _product_search_string: function (product) { // TODO: supported search with name_second
            ...
            let lots = self.posmodel.lot_by_product_id[product.id];
        ...
        get_product_string: function (product) {
            let label = product['default_code'] || '';
            if (product['name']) {
                if (!label) {
                    label = product['name'];
                } else {
        ...
        get_product_by_category: function (category_id) {
            ...
            console.log('[get_product_by_category] total products return: ' + list.length)
            return list
        },
        // save pos orders
        ...
        save_pos_orders: function (orders) {
            this.orders_count = 0;
            this.order_by_partner_id = {};
            let branch_id = self.posmodel.config.pos_branch_id;
            let pos_orders_filter_by_branch = self.posmodel.config.pos_orders_filter_by_branch;
            ...
            for (let i = 0; i < orders.length; i++) {
                ...
                this.lines_by_order_id[order['id']] = []
                this.orders_count += 1;
                if (order.partner_id) {
        ...
        save_invoices: function (invoices) {
            for (let i = 0; i < invoices.length; i++) {
                ...
                this.lines_invoice_by_id[invoice.id] = []
        ...
        save_invoice_lines: function (moveLines) {
            for (let i = 0; i < moveLines.length; i++) {
                ...
                this.invoice_line_by_id[moveLine['id']] = moveLine;
                this.lines_invoice_by_id[moveLine.move_id[0]].push(moveLine)
            }
        },
