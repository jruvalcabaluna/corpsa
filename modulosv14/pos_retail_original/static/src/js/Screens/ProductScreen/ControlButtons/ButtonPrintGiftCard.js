odoo.define('pos_retail.ButtonPrintGiftCard', function (require) {
        ...
        async onClick() {
            ...
            if (confirmed && selectedItems['items'].length > 0) {
                ...
                if (confirmed) {
                    ...
                    let validate = await this.env.pos._validate_action(this.env._t(' Need create ') + totalGiftNumber + this.env._t(' Gift Card, with Amount fixed is: ') + this.env.pos.format_currency(couponProgram.reward.discount_fixed_amount));
                    if (!validate) {
                        return false;
                    } else {
    ...
    ProductScreen.addControlButton({
        ...
        condition: function () {
            return this.env.pos.couponGiftCardTemplate && this.env.pos.couponGiftCardTemplate.length > 0;
        },
    });
