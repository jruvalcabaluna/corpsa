    ...
    const RetailProductScreen = (ProductScreen) =>
        class extends ProductScreen {
            constructor() {
                ...
                this.state = useState({cashControl: status, numpadMode: 'quantity', screen: 'Products'});
            }
            ...
            _setValue(val) {
                if (this.currentOrder.finalized || !this.env.pos.config.allow_qty || this.state.screen != 'Products' || (this.currentOrder.get_selected_orderline() && ((this.state.numpadMode === 'quantity' && this.env.pos.config.validate_quantity_change) || (this.state.numpadMode === 'discount' && this.env.pos.config.validate_quantity_change) || (this.state.numpadMode === 'price' && this.env.pos.config.validate_quantity_change)))) {
                    console.warn('[Screen products state is not Products] or [POS active validation] reject trigger event keyboard]')
                    return false
                } else {
                    super._setValue(val)
                }
            ...
            async _keyboardHandler() {
                ...
                for (let i = 0; i < this.buffered_key_events.length; i++) {
                    ...
                    //     this.env.pos.alert_message({
                    //         title: this.env._t('Mode change to [Discount]'),
                    //         body: this.env._t('Active set a Discount'),
                    //         color: 'success'
                    //     })
                    //     keyAccept = true
                    // }
                    ...
                    //     this.env.pos.alert_message({
                    //         title: this.env._t('Mode change to [Price]'),
                    //         body: this.env._t('Active set a Price'),
                    //         color: 'success'
                    //     })
                    //     keyAccept = true
                    // }
                    ...
                    //     this.env.pos.alert_message({
                    //         title: this.env._t('Mode change to [Quantity]'),
                    //         body: this.env._t('Active set a Quantity'),
                    //         color: 'success'
                    //     })
                    //     keyAccept = true
                    // }
            ...
            async _onClickPay() {
                ...
                if (selectedOrder && selectedOrder.get_total_with_tax() > 0 && !selectedOrder.is_return) {
                    this.env.pos.automaticSetCoupon()
                }
                if (selectedOrder && selectedOrder.get_total_with_tax() == 0) {
                    let {confirmed, payload: result} = await this.showPopup('ConfirmPopup', {
                        title: this.env._t('Total need paid is Zero: ') + this.env.pos.format_currency(0),
                        body: this.env._t('Are you wanted submit Order with Amount Total is:') + this.env.pos.format_currency(0),
                    })
                    if (!confirmed) {
                        return true
                    }
                ...
                if (this.env.isMobile) {
                    ...
                    this.state['screen'] = 'Payment'; // single screen
                }
                    ...
                    if (packLotLinesToEdit && packLotLinesToEdit.length) {
                        ...
                        if (confirmed && selectedItems['items'].length > 0) {
                            ...
                            return selectedOrder.add_product(event.detail, {
                                draftPackLotLines,
                                description: 'Auto full fill lot',
                                price_extra: 0,
                                quantity: 1,
                            });
                        } else {
